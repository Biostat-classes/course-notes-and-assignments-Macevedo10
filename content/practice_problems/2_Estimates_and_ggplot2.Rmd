...
xlab("TotalSleep")+
ylab("Total Sleep")+
ggtitle("The effect of Brain Weight on decreasing Total Sleep")
```
...
`{ggplot(sleep, aes(x=BrainWt, y =TotalSleep)) +}   geom_point(aes(colour=Taxa))`
...
We can see a right skewed graph based on total sleep including all
subjects.
...
ggplot(sleep, aes(x=Dreaming)) +
  ...
  ylab("Hours") +
  xlab("Total Sleep") +
  ggtitle("Variations of total sleep in primates")
```

```{r}
ggplot(sleep[sleep$Taxa == "Primate",], aes(x=TotalSleep)) +
  geom_histogram() + 
          ylab("Frequency of sleep in Primtes") +
    xlab("Total Sleep") +
    ggtitle("Variations of total sleep in primates")
```
...
```{r}
str(sleep)
sleep$TotalSleep <-factor(sleep$TotalSleep)
```

```{r}
levels(sleep$TotalSleep)
```
...
A 95% confidence interval means that if you have 100 samples that 95 of
them contain the true mean or are accurate while 5 of them may not have
the true mean or be accurateand are far from the true mean and data
overall.
