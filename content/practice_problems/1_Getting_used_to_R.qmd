...
The : means give a list of integers between the given numbers.

#### 2

Now try to guess the output of these commands

-   length(x) Tells you amount of numbers in the interval of x
-   max(x) Tells you the biggest number in data set
-   x\[x \< 10\] tells you within the data set of x the numbers that are
    smaller than 10
-   x\^2 tells you the amount of each number in the data set when they
    are squared
-   x\[ x \< 12 & x \> 7\] Figures out the numbers that fulfill these
    requirements between 7 and 12
...
-1:2 shows that it is an integer starting from negative one and going to
2. (-1):2 does the same thing as -1:2. -(1:2) makes the two integers
inside the parenthesis negative and gives the list of numbers between
these two integers.
...
It has 62 rows. Each row depicts the information gathered from the sleep
study of a different animal.
...
```{r}
rename(sleep, Body_weight = BodyWt)
```
...
```{r, eval=F}
sleep <- read.csv("http://raw.githubusercontent.com/jsgosnell/CUNY-BioStats/master/datasets/sleep.csv", stringsAsFactors = 
```

```{r}
plot(species,(total sleep)data = sleep)
```
...
```{r}
plot(TotalSleep ~ factor(Primate), data = sleep)
```
...
```{r}
plot(TotalSleep ~ factor(BrainWt), data = sleep)
```

```{r}
summary(plot(TotalSleep ~ factor(Primate), data = sleep))
```
